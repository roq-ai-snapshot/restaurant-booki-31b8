...
const notificationMapping: Record<string, NotificationConfigInterface> = {
  'Order #123 has been placed by a customer.create': {
    roles: ['chichi-owner', 'chichi-manager', 'chichi-staff', 'chichi-chef'],
    key: 'order_create',
    tenantPath: [],
    userPath: [],
  },
  'Inventory for item #456 has been updated.update': {
    roles: ['chichi-owner', 'chichi-manager'],
    key: 'inventory_update',
    tenantPath: [],
    userPath: [],
  },
  'Feedback #789 has been received from a customer.create': {
    roles: ['chichi-owner', 'chichi-manager', 'chichi-staff'],
    key: 'feedback_create',
    tenantPath: [],
    userPath: [],
  },
...
const ownerRoles: string[] = ['chichi-owner'];
const customerRoles: string[] = ['chichi-visitors'];
const tenantRoles: string[] = ['chichi-owner', 'chichi-manager', 'chichi-staff', 'chichi-chef', 'chichi-customer'];
...
export async function notificationHandlerMiddleware(req: NextApiRequest, entityId: string) {
  ...
  const tenant = await prisma.chichi.findFirst({
