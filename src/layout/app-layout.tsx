...
import { getChichis } from 'apiSdk/chichis';
...
const SidebarContent = ({ onClose, ...rest }: SidebarProps) => {
  ...
  const tenantRoles = ['chichi-owner', 'chichi-manager', 'chichi-staff', 'chichi-chef', 'chichi-customer'];
  const isTenantUser = tenantRoles.some((role) => session?.user?.roles?.includes(role));
  const { data, error, isLoading } = useSWR<ChichiInterface[]>(
    () => (isTenantUser && session?.user?.tenantId ? `/chichis` : null),
    () => getChichis({ tenant_id: session?.user?.tenantId }).then(({ data }) => data),
  );
  loading = isLoading;
  ...
  const MockedLinkItems: Array<NavItemPropsInterface> = [
    ...
    {
      name: 'Chichis',
      path: isTenantUser ? `/chichis/view/${data?.[0]?.id}` : '/chichis',
      entity: 'chichi',
