...
import { getChichis, deleteChichiById } from 'apiSdk/chichis';
import { ChichiInterface } from 'interfaces/chichi';

type ColumnType = ColumnDef<ChichiInterface, unknown>;

interface ChichiListPageProps {
...
export function ChichiListPage(props: ChichiListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<ChichiInterface>>(
    () => `/chichis?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: ChichiInterface) => {
    if (hasAccess('chichi', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/chichis/view/${row.id}`);
    }
  };
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/chichis/view/${record.id}`} passHref legacyBehavior>
                <Button
              ...
              {hasAccess('chichi', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                <NextLink href={`/chichis/edit/${record.id}`} passHref legacyBehavior>
                  <Button
                    onClick={(e) => e.stopPropagation()}
                    mr={2}
                    padding="0rem 0.5rem"
              ...
              {hasAccess('chichi', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                <IconButton
                  onClick={(e) => {
                    e.stopPropagation();
